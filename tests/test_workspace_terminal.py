...
def test_native_workspace_diffs_copy_and_no_execution(tmp_path, repository):  # noqa: F811
    ...
    try:
        ...
        probe.send(b"Side-by-side\r")
        probe.wait("Workspace diff · side by side")
        assert any("-base" in row and "+staged" in row for row in probe.screen.display)
        capture(probe, "approachability-side-by-side")
