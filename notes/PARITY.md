...
Current disposition: 2026-09-18, after PARITY-01–07. The active readiness table in
[PLAN](PLAN.md) owns remaining work; old delivery tables are historical receipts.
...
| Return | Directory-local canonical TUI resume, validated graceful-interruption resume, public-context turn branches and recovery; exact-directory CLI reference/adoption; explicit CLI handoff for original CLI identity | Native original-identity CLI migration, exact uncertain-context repair and arbitrary module-private state |
...
| Administer and script | Pinned CLI entrypoint for setup, bundles/modules/sources, routing, notifications, update/reset and canonical CLI sessions; installed prompt/stdin/text/JSON/trace and read-only shell completion; native clear/branch/export/direct tools and reversible archive | CLI-store operations are not native-store mutations; not every wizard has a second native implementation |
...
| Install | Private pinned Git installation; published rc5 compiler-free wheels with source/digest receipts; five-platform native/installed-PTY gates including Ubuntu 22.04, actual Mac PNG pasteboard and exact CI ARM installed-live proof | Physical-terminal/live breadth beyond tested gates; musl; Git source install still needs Rust/linker |
...
Current readiness work is tracked once in [PLAN](PLAN.md): release/upgrade delivery,
bounded service/account verification, explained CLI measurements and terminal breadth.
