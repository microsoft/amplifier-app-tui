# Amplifier TUI — historical development walkthrough

This preserved walkthrough describes successive development slices, not one current
release. Start with the [current README](README.md) and [first conversation](docs/GETTING-STARTED.md).
Historical verification claims apply only to the revisions named in their evidence.
...
New launches use your CLI's global/project/local settings: active bundle, app behaviors,
providers, routing, permissions and declared service destinations. Configuration is read,
not copied or rewritten; authentication remains provider-owned. Complete interactive
provider login in the CLI before launching. Configured hooks may run or contact services
at startup. `--settings-policy isolated` opts out of CLI settings. Explicit `--preset`,
`--bundle` or `--overlay` also retains isolated policy unless paired with
`--settings-policy cli`. The isolated default needs `ANTHROPIC_API_KEY`.
`--fixture` is a credential-free scripted-provider
...
This is **live execution**, not the design scene. Ordinary new launches use CLI settings
for the selected working directory. Model calls incur charges and configured hooks keep
their declared destinations. Use `--settings-policy isolated --preset anchors-amp-dev`
for the isolated development preset (Anthropic/Haiku), or trusted `--bundle` / `--overlay`
configuration. `--cwd` selects both the tool workspace and project settings scope.
Old conversations retain their saved policy; configuration changes can refuse resume.
...
Actions shows the module's startup discovery catalog without loading a skill.
`/skill NAME arguments` uses the CLI's skill-loading prompt semantics; Tab completes
discovered names. Unknown slash commands refuse locally rather than becoming prompts. Ask the
