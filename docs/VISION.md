...
Amplifier TUI is an interchangeable client for Amplifier CLI users, with a polished terminal workspace as responsive as the CLI. Terminal and web hosts share Foundation's native history and writer ownership: launch in a project, choose an existing conversation, continue under the same identity, then return in another client without import, conversion or lost history. Canonical transcript and metadata retain conversation state; Context Intelligence enriches activity without becoming execution authority or another transcript. Full CLI workflow and ecosystem host-service parity is the baseline; TUI enhancements are additive. Administration and scripting reuse CLI policy, while interactive approvals, diagnostics, completion, goal safety and child execution belong to the terminal host. People inspect loaded configuration without invoking a model or exposing credentials. Unsupported control transitions remain explicit; domain-specific dashboards are not prerequisites for compatibility.
Amplifier source dependencies and default module sources follow their canonical main branches. Resolved commits are recorded as qualification evidence; they do not freeze future installations. Compatibility checks preserve host policy as those sources advance. It learns from Codex's proven interaction and terminal behavior while Amplifier's ecosystem owns execution.
Every live conversation uses the native shared store from creation onward. Returning to a conversation does not depend on its originating client, a TUI journal, or import-size limits. Canonical messages supply history; configured shared event logs enrich the ordinary Activity view in memory. Presentation limits bound previews, never admission of valid native history.
...
Visible actions and contextual choices make the experience usable without memorizing shortcuts. The conversation outlives screens and engines; returning offers a picker, search and input recall scoped to the resolved launch directory, not its parents or children. Resume never silently changes the workspace.
Viewing a conversation is separate from owning its execution. A busy conversation remains readable with one ownership notice and an explicit Continue here action. Cooperating owners finish current calls, save and clean up before yielding; the successor acquires and reloads before accepting work. Sessions release ownership after five minutes with neither runtime work nor observed user activity, without losing the terminal view or draft. Editing and inspection count as activity; background painting and polling do not. Reacquisition validates current history and configuration rather than trusting a stale mount. Handoff never steals a lock, submits a retained draft, or transfers unfinished operations as though they completed.
...
The ordinary conversation accumulates native terminal history; selection and tmux scrolling need no special view. Full-height presentation does not archive unused startup rows or temporary layout padding as conversation history.
...
Markdown and syntax-coloured code stay readable without changing copied source. Wrapped and nested lists retain their structure, quoted lines retain their scope, and headings use accent contrast, typography and block spacing without displaying Markdown delimiters. Major sections remain distinct when a terminal's bold font is imperceptible; colour-free presentation retains underlining. Links avoid redundant destinations without hiding a target that the terminal cannot otherwise open. Chronological recall, immutable attachment sets, semantic file/line references and external editing preserve unsent intent. Referenced excerpts retain their source version and location through waiting work and return, without a hidden reread.
...
The person's draft, admitted pending intent and the agent's history have different owners; portable controls require a supported shared contract, never guessed restoration from telemetry.
