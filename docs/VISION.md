...
Amplifier TUI is an interchangeable client for Amplifier CLI users, with a polished terminal workspace as responsive as the CLI. Both clients use the same configuration and canonical session data: launch in a project, choose an existing CLI conversation, continue in the TUI, then return to the CLI under the same identity without import, conversion or lost history. Full CLI workflow and ecosystem host-service parity is the baseline; TUI enhancements are additive, not a separate authoritative conversation store. Administration and scripting reuse CLI policy, while interactive approvals, diagnostics, completion, goal safety and child execution belong to the terminal host. People can inspect loaded configuration without invoking a model or exposing credentials. A mounted bundle or command handoff is not a claim of complete interactive parity; unsupported transitions remain explicit. Domain-specific dashboards are not prerequisites for compatibility.
