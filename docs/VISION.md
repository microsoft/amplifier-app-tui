...
Viewing a conversation is separate from owning its execution. A busy conversation remains readable with one ownership notice and an explicit Continue here action. Cooperating owners finish current calls, save and clean up before yielding; the successor acquires and reloads before accepting work. Quiet, settled sessions release ownership without losing the terminal view or draft. Reacquisition validates current history and configuration rather than trusting a stale mount. Handoff never steals a lock, submits a retained draft, or transfers unfinished operations as though they completed.
