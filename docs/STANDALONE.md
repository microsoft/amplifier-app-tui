...
Open your project directory and run `amplifier-tui --standalone`. Ordinary new launches use your
...
To continue existing CLI work, run `amplifier-tui --standalone --resume` in
...
See [switching limits](MIGRATION.md).

Use `--settings-policy isolated` to opt out of CLI configuration; its default preset
needs `ANTHROPIC_API_KEY`. It uses an isolated Amplifier home under `--state-dir/foundation`
unless `--cli-home` is explicit; return with that `--cli-home`. All live configurations
use the same CLI session format. Explicit `--preset`, `--bundle` or `--overlay` also defaults
...
context is incomplete, shared sessions require inspection/export. Originals and unknown effects
...
`amplifier-tui --standalone --resume` opens a picker. Reopening does not replay tools or release
...
- [Your first conversation and controls](GETTING-STARTED.md)
- [Resume, migration, provider login and session controls](MIGRATION.md)
- [Troubleshooting and safe reports](SUPPORT.md)
- [Account and service verification boundaries](SERVICE-VERIFICATION.md)
...
The [Amplifier Unified handoff](UNIFIED-HANDOFF.md) maps the shared-session
implementation, verification commands and remaining cross-client work.

Read [AGENTS.md](../AGENTS.md), [vision](VISION.md), [plan](../notes/PLAN.md) and
